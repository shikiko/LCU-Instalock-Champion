...
gamedirs = [r'E:\GarenaLOL\32771\LeagueClient',
    ...
    for champion in champions:
        if champions[champion] == argv:
            if argv in championNames:
                ...
                print (Back.RED + Fore.WHITE + 'Invalid champion',
                       argv, Style.RESET_ALL)
                exit()
            break
...
for champion in championsPrio:
    if champion not in championIds:
        print (Back.RED + Fore.WHITE + 'Invalid champion ID', champion,
               Style.RESET_ALL)
        exit()
...
userpass = b64encode(bytes('%s:%s' % (username, password), 'utf-8'
                     )).decode('ascii')
...
while True:
    ...
    if r.status_code != 200:
        print (Back.BLACK + Fore.RED + str(r.status_code)
    ...
    # Pick/lock champion
        ...
        for action in cs['actions'][0]:
            ...
            if action['championId'] == 0:
                ...
                url = '/lol-champ-select/v1/session/actions/%d' \
                    % action['id']
